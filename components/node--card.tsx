        ...
        <article className="grid bg-secondary-10 rounded-lg md:grid-cols-2" {...props}>
            {node.field_article_image && (
                <Link href={node.path.alias} passHref>
                    <a className="block overflow-hidden no-underline rounded-t-lg md:rounded-l-lg object-fill">
...
export function NodeCardSmall({ node, ...props }) {
    ...
    return (
        <article className="grid bg-secondary-10 rounded-lg grid-cols-1 relative mt-[90px] md:m-0" {...props}>
            {node.field_article_image && (
                <Link href={node.path.alias} passHref>
                    <a className="block overflow-hidden object-fill absolute top-[-90px] left-1/2 translate-x-[-50%] shieldMask aspect-{95 / 122} w-3/5 md:w-2/5 md:left-[-20%] md:translate-x-[0] md:top-0 max-h-full">
