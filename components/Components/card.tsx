...
export function NodeCard({
  ...
  if (node.field_inline_link) {
    itemContent = (
      <CardBody node={node} className={className} linkInside={true}/>
    )
  }
  return (
    <article className="relative rounded-lg" {...props}>
      {itemContent}
    </article>
  );
}
...
function CardBody({
  ...
  return (
    <>
      <div className={classNames('flex justify-between flex-col w-full', {'lg:flex-row': imageResponsive})}>
        {image && (
...
function Time() {
  return (
    <div className="relative -mt-6 flex justify-center">
      <p className="text-xl text-white-900 rounded-full bg-primary-900 px-2 py-4 w-[65px] h-[65px]">19:00</p>
    </div>
  )
}
